...
REAL(KIND=JPRB),OPTIONAL     :: PGP2(:,:,:) !,INTENT(IN)
...
INTEGER(KIND=JPIM) :: maxpgp2_pars,maxpgpuv_pars,maxpgp3a_pars,maxpgp3b_pars,maxpgpuv_levs,maxpgp3a_levs,maxpgp3b_levs

INTEGER(KIND=JPIM) :: pgpuv_lbounds(4),pgpuv_ubounds(4),pgpuv_size(4)
integer i,k,m
!     ------------------------------------------------------------------
...
DO INS=1,KNSEND
  ...
  ISEND_FLD_START=ZCOMBUFS(-1,INS)
  ISEND_FLD_END = ZCOMBUFS(0,INS)
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IFLDT,JBLK,IFIRST,ILAST,JK,JJ,JI)
  DO JJ=ISEND_FLD_START,ISEND_FLD_END
    ...
    DO JBLK=1,NGPBLKS
      ...
      IF(IFIRST > 0) THEN
        ILAST = KGPTRSEND(2,JBLK,ISETW(INS))
        JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)-IFIRST+1
        IF(LLINDER) THEN
          DO JK=IFIRST,ILAST
            ZCOMBUFS(JI+JK,INS) = PGP(JK,KPTRGP(IFLDT),JBLK)
          ENDDO
        ELSE
          IF(LLPGPONLY) THEN
            DO JK=IFIRST,ILAST
              ZCOMBUFS(JI+JK,INS) = PGP(JK,IFLDT,JBLK)
            ENDDO
          ELSEIF(LLUV(IFLDT)) THEN
             DO JK=IFIRST,ILAST
                ZCOMBUFS(JI+JK,INS) = PGPUV(JK,IUVLEVS(IFLDT),IUVPARS(IFLDT),JBLK)
             ENDDO
          ELSEIF(LLGP2(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              ZCOMBUFS(JI+JK,INS) = PGP2(JK,IGP2PARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP3A(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              ZCOMBUFS(JI+JK,INS) = PGP3A(JK,IGP3ALEVS(IFLDT),IGP3APARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP3B(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              ZCOMBUFS(JI+JK,INS) = PGP3B(JK,IGP3BLEVS(IFLDT),IGP3BPARS(IFLDT),JBLK)
            ENDDO
          ENDIF
        ENDIF
      ENDIF
...
!  print *,'maxpgp2_pars=',maxpgp2_pars
!  print *,'maxpgpuv_pars=',maxpgpuv_pars
  !print *,'maxpgp3a_pars=',maxpgp3a_pars
!  print *,'maxpgp3b_pars=',maxpgp3b_pars
!  print *,'maxpgpuv_levs=',maxpgpuv_levs
!  print *,'maxpgp3a_levs=',maxpgp3a_levs
!  print *,'maxpgp3b_levs=',maxpgp3b_levs
