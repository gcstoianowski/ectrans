...
IF ( .NOT. ALLOCATED(ZCOMBUFS_HEAP) ) THEN
  ...
  INSEND_PREV = KNSEND
ELSEIF ( KSENDCOUNT /= ISENDCOUNT_PREV .OR. KNSEND /= INSEND_PREV ) THEN
...
IF ( .NOT. ALLOCATED(ZCOMBUFR_HEAP) ) THEN
  ...
  INRECV_PREV = KNRECV
ELSEIF ( KRECVCOUNT /= IRECVCOUNT_PREV .OR. KNRECV /= INRECV_PREV ) THEN
...
!
USE TIMING_MOD,    ONLY: TCOMM1, TCOMM2, TPACK1, TPACK2, TRECV1, TRECV2, &
...
if (tenable) then
  T_EVENT(TCOUNT) = GET_TIME()
  T_BATCH(TCOUNT) = KSENDTOT(MYPROC)
  T_STAGE(TCOUNT) = 0
