...
INTEGER(KIND=JPIM),INTENT(IN) :: KF_FS,KF_GP,SEND_ID
REAL(KIND=JPRB), CONTIGUOUS, INTENT(OUT)   :: PGLAT(:,:)  !(KF_FS,D%NLENGTF)
INTEGER(KIND=JPIM),INTENT(IN) :: KVSET(KF_GP)
...
INTEGER(KIND=JPIM) :: ISEND_FLD_END(KNSEND)
...
INTEGER(KIND=JPIM) :: IFLDOFF(KF_FS),ierr,dest
...
DO INR=1,KNRECV
  IRECV=KRECV(INR)
  CALL MPI_IRECV(PCOMBUFR(IOFFRECV:IOFFRECV+KRECVTOT(IRECV)-1,INR), KRECVTOT(IRECV),MPI_REAL,NPRCIDS(IRECV)-1, &
       & ITAG,MPI_COMM_WORLD,KREQ_RECV(INR),IERR)
  !  CALL MPL_RECV(PCOMBUFR(IOFFRECV:IOFFRECV+KRECVTOT(IRECV)-1,INR), &
!        & KSOURCE=NPRCIDS(IRECV), &
!        & KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=KREQ_RECV(INR), &
...
!DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            ...
            PGLAT(KINDEX(IPOS),JFLD64) = PGP(JK,IFLD,JBLK)
          ENDDO
        ENDDO
      ELSE
        DO JFLD64=1,IFLDS
...
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              PGLAT(KINDEX(IPOS),JFLD64) = PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK)
            ENDDO
          ELSEIF(LLGP2(IFLD)) THEN
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              PGLAT(KINDEX(IPOS),JFLD64) = PGP2(JK,IGP2PARS(IFLD),JBLK)
            ENDDO
          ELSEIF(LLGP3A(IFLD)) THEN
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              PGLAT(KINDEX(IPOS),JFLD64) = PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK)
            ENDDO
          ELSEIF(LLGP3B(IFLD)) THEN
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              PGLAT(KINDEX(IPOS),JFLD64) = PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK)
            ENDDO
          ELSE
...
!$OMP PARALLEL PRIVATE(JBLK,IFIRST,ILAST,INS,ISEND,ISETA,ISETB,&
...
INTEGER(KIND=JPIM), INTENT(INOUT) :: KREQ_SEND(:),KREQ_RECV(:)
