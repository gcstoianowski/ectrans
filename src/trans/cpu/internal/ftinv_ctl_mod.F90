...
INTEGER(KIND=JPIM), SAVE :: KFFS_PREV = -1
INTEGER(KIND=JPIM), SAVE :: NLEN_PREV = -1
...
IF (NSTACK_MEMORY_TR == 1) THEN
  ZGTF => ZGTF_STACK(:,:)
ELSE
  IF (.NOT. ALLOCATED(ZGTF_HEAP)) THEN
...
! to be used which is an OPEN-MP loop, that would cause a threads synchronization lock :
    KFFS_PREV=KF_FS
    NLEN_PREV=D%NLENGTF
  ELSEIF (KF_FS .NE. KFFS_PREV .OR. D%NLENGTF .NE. NLEN_PREV) THEN
    DEALLOCATE(ZGTF_HEAP)
    ALLOCATE(ZGTF_HEAP(KF_FS,D%NLENGTF))
    KFFS_PREV=KF_FS
    NLEN_PREV=D%NLENGTF
  ENDIF
