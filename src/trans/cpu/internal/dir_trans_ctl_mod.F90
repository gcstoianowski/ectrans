...
if (tenable) then
    T_EVENT(TCOUNT) = GET_TIME()
    T_BATCH(TCOUNT) = D%NDGL_FS
    T_STAGE(TCOUNT) = THISBATCH%NF_FS
 ...
 if(luse_progress_thread) then
       ...
       ! moved from TRGTOL_COMM_RECV
       CALL MPI_WAITALL(THISBATCH%NNSEND,THISBATCH%IREQ_SEND,MPI_STATUSES_IGNORE,IERR)
    endif
