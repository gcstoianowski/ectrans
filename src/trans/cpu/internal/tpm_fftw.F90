...
PRIVATE
PUBLIC CREATE_PLAN_FFTW, CREATE_PLAN_EFFTW, DESTROY_PLAN_FFTW, DESTROY_PLANS_FFTW, INIT_PLANS_FFTW, &
...
SUBROUTINE CREATE_PLAN_EFFTW(KPLAN,KTYPE,KN,KLOT,embd1)
INTEGER(KIND=JPIB),INTENT(OUT) :: KPLAN
INTEGER(KIND=JPIM),INTENT(IN) :: KTYPE,KN,KLOT,embd1

INTEGER(KIND=JPIB) :: IPLAN
INTEGER(KIND=JPIM) :: IRANK, ISTRIDE
INTEGER(KIND=JPIM) :: JL, JN
INTEGER(KIND=JPIM) :: IRDIST,ICDIST,IN(1),IEMBED1(1),IEMBED2(1)
REAL(KIND=JPRB), POINTER :: ZDUM1(:),ZDUM2(:)
TYPE(C_PTR) :: ZDUMP1,ZDUMP2
LOGICAL :: LLFOUND
LOGICAL :: LLRESTRICT_PLANS=.TRUE.
TYPE(FFTW_PLAN),POINTER :: CURR_FFTW_PLAN,START_FFTW_PLAN
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE, ZHOOK_HANDLE2
...
ISTRIDE=1
IN(1)=KN
IEMBED1(1)=embd1     !IN(1)
ICDIST=KN/2+1
IRDIST=embd1 !   ICDIST*2
IEMBED2(1) = ICDIST
...
!     WRITE(*,'("CREATE_PLAN_FFTW: END: DESTROYING A PLAN AT THE START OF THE LIST")')
  ...
  ENDIF
  IF (JPRB == JPRD) THEN
     ZDUMP1=FFTW_ALLOC_COMPLEX(INT(1,C_SIZE_T))
     ZDUMP2=FFTW_ALLOC_COMPLEX(INT(1,C_SIZE_T))
  ELSE
     ZDUMP1=FFTWF_ALLOC_COMPLEX(INT(1,C_SIZE_T))
     ZDUMP2=FFTWF_ALLOC_COMPLEX(INT(1,C_SIZE_T))
  END IF
  CALL C_F_POINTER(ZDUMP1,ZDUM1,[2])
  CALL C_F_POINTER(ZDUMP2,ZDUM2,[2])
  IF( KTYPE==1 )THEN
     IF (LHOOK) CALL DR_HOOK('FFTW_PLAN_MANY_DFT_C2R',0,ZHOOK_HANDLE2)
     IF (JPRB == JPRD) THEN
        CALL DFFTW_PLAN_MANY_DFT_C2R(IPLAN,IRANK,IN,KLOT,ZDUM1,IEMBED1,ISTRIDE,ICDIST,&
             & ZDUM1,IEMBED1,ISTRIDE,IRDIST,FFTW_ESTIMATE+FFTW_NO_SIMD)
     ELSE
        CALL SFFTW_PLAN_MANY_DFT_C2R(IPLAN,IRANK,IN,KLOT,ZDUM1,IEMBED1,ISTRIDE,ICDIST,&
             & ZDUM1,IEMBED1,ISTRIDE,IRDIST,FFTW_ESTIMATE+FFTW_NO_SIMD)
     END IF
     IF (LHOOK) CALL DR_HOOK('FFTW_PLAN_MANY_DFT_C2R',1,ZHOOK_HANDLE2)
  ELSEIF( KTYPE==-1 )THEN
     IF (LHOOK) CALL DR_HOOK('FFTW_PLAN_MANY_DFT_R2C',0,ZHOOK_HANDLE2)
     IF (JPRB == JPRD) THEN
        CALL DFFTW_PLAN_MANY_DFT_R2C(IPLAN,IRANK,IN,KLOT,ZDUM1,IEMBED1,ISTRIDE,IRDIST,&
             & ZDUM2,IEMBED2,ISTRIDE,ICDIST,FFTW_ESTIMATE+FFTW_NO_SIMD)
     ELSE
        CALL SFFTW_PLAN_MANY_DFT_R2C(IPLAN,IRANK,IN,KLOT,ZDUM1,IEMBED1,ISTRIDE,IRDIST,&
             & ZDUM2,IEMBED2,ISTRIDE,ICDIST,FFTW_ESTIMATE+FFTW_NO_SIMD)       
     END IF
     IF (LHOOK) CALL DR_HOOK('FFTW_PLAN_MANY_DFT_R2C',1,ZHOOK_HANDLE2)
  ELSE
  ...
  ENDIF
  IF (JPRB == JPRD) THEN
     CALL FFTW_FREE(ZDUMP1)
     CALL FFTW_FREE(ZDUMP2)
  ELSE
     CALL FFTWF_FREE(ZDUMP1)
     CALL FFTWF_FREE(ZDUMP2)
  END IF
  KPLAN=IPLAN
  TW%N_PLANS(KN)=TW%N_PLANS(KN)+1
  IF( TW%N_PLANS(KN) /= 1 )THEN
...
IF (LHOOK) CALL DR_HOOK('CREATE_PLAN_FFTW',1,ZHOOK_HANDLE)
RETURN
END SUBROUTINE CREATE_PLAN_EFFTW


SUBROUTINE EXEC_EFFTW(KTYPE,KRLEN,KCLEN,KOFF,KFIELDS,LD_ALL,PIN,PREEL)
...
IF( LD_ALL )THEN
  CALL CREATE_PLAN_EFFTW(IPLAN_C2R,KTYPE,KRLEN,KFIELDS,dim1)
  IF  (JPRB == JPRD) THEN
     ZFFTP=FFTW_ALLOC_COMPLEX(INT(KCLEN/2*KFIELDS,C_SIZE_T))
  ELSE
     ZFFTP=FFTWF_ALLOC_COMPLEX(INT(KCLEN/2*KFIELDS,C_SIZE_T))
...
!call gstats(1500,1)
    IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_R2C',0,ZHOOK_HANDLE2)
    IF (JPRB == JPRD) THEN
       CALL DFFTW_EXECUTE_DFT_R2C(IPLAN_C2R,PIN(KOFF,1),ZFFT)
    ELSE
       CALL SFFTW_EXECUTE_DFT_R2C(IPLAN_C2R,PIN(KOFF,1),ZFFT)
    END IF
    IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_R2C',1,ZHOOK_HANDLE2)
call gstats(1501,0)
    DO JJ=1,KCLEN
