...
MODULE FSCAD_MOD
  ...
  PRIVATE
  PUBLIC :: FSCAD, PREPARE_FSCAD, FSCAD_HANDLE

  TYPE FSCAD_HANDLE
  END TYPE

CONTAINS
  FUNCTION PREPARE_FSCAD(ALLOCATOR) RESULT(HFSC)
    IMPLICIT NONE

    TYPE(BUFFERED_ALLOCATOR), INTENT(INOUT) :: ALLOCATOR
    TYPE(FSCAD_HANDLE) :: HFSC
  END FUNCTION
SUBROUTINE FSCAD(ALLOCATOR,HFSC,PREEL_COMPLEX, KF_FS, KF_UV, KF_SCALARS, KUV_OFFSET, &
        & KSCALARS_OFFSET, KSCALARS_NSDER_OFFSET, KUV_EWDER_OFFSET, KSCALARS_EWDER_OFFSET)

!**** *FSCAD - Adjoint of division by a*cos(theta), east-west derivatives computation
...
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IGLG,IOFF_LAT,IOFF_UV,IOFF_UV_EWDER,&
  !$ACC&                                                RET_REAL,RET_COMPLEX,ZACHTE2,JM,JF,KGL) &
        ...
        IF (JM <= G_NLOEN(IGLG)/2) THEN
          ...
          IF (JM <= G_NMEN(IGLG)) THEN
            ...
            PREEL_COMPLEX(IOFF_UV+2*JM+2) = &
              & PREEL_COMPLEX(IOFF_UV+2*JM+2) - PREEL_COMPLEX(IOFF_UV_EWDER+2*JM+1)*ZACHTE2*REAL(JM,JPRBT)
            PREEL_COMPLEX(IOFF_UV+2*JM+1) = &
              & PREEL_COMPLEX(IOFF_UV+2*JM+1) + PREEL_COMPLEX(IOFF_UV_EWDER+2*JM+2)*ZACHTE2*REAL(JM,JPRBT)
          ENDIF
        ENDIF
      ENDDO
    ENDDO
...
#if defined (HOSTGPU)
  !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) &
  !$OMP& PRIVATE(IGLG,IOFF_LAT,IOFF_SCALARS_EWDER,IOFF_SCALARS,ZACHTE2) &
  !$OMP& SHARED(IBEG,IEND,IINC,KF_SCALARS,R,ILOEN_MAX,OFFSET_VAR,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET,G,D,&
  !$OMP&        KF_FS,F,PREEL_COMPLEX)
#elif defined (OMPGPU)
!! #ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) &
  !$OMP& PRIVATE(IGLG,IOFF_LAT,IOFF_SCALARS_EWDER,IOFF_SCALARS,ZACHTE2) &
  !$OMP& SHARED(IBEG,IEND,IINC,KF_SCALARS,R,ILOEN_MAX,OFFSET_VAR,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET,G,D,&
  !$OMP&        KF_FS,F,PREEL_COMPLEX) &
  !$OMP& MAP(TO:IBEG,IEND,IINC,KF_SCALARS,ILOEN_MAX,OFFSET_VAR,KF_FS,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET)
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IGLG,IOFF_LAT,IOFF_SCALARS_EWDER,&
  !$ACC&                                                IOFF_SCALARS,ZACHTE2,KGL,JF,JM) &
  !$ACC& FIRSTPRIVATE(IBEG,IEND,IINC,OFFSET_VAR,KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET,KF_SCALARS,KF_FS,ILOEN_MAX) &
#ifdef _CRAYFTN
  !! NOTE: These asynchronous kernels are triggering the error: HIPFFT_PARSE_ERROR
  !$ACC&
#else
      ...
      DO JM=0,ILOEN_MAX/2
        ...
        IF (JM <= G_NLOEN(IGLG)/2) THEN
          ...
          IF (JM <= G_NMEN(IGLG)) THEN
            ...
            PREEL_COMPLEX(IOFF_SCALARS+2*JM+2) = &
              & PREEL_COMPLEX(IOFF_SCALARS+2*JM+2) - PREEL_COMPLEX(IOFF_SCALARS_EWDER+2*JM+1)*ZACHTE2*REAL(JM,JPRBT)
            PREEL_COMPLEX(IOFF_SCALARS+2*JM+1) = &
              & PREEL_COMPLEX(IOFF_SCALARS+2*JM+1) + PREEL_COMPLEX(IOFF_SCALARS_EWDER+2*JM+2)*ZACHTE2*REAL(JM,JPRBT)
          ENDIF
        ENDIF
      ENDDO
    ENDDO
