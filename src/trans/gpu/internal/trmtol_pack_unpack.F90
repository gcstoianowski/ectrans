...
CONTAINS
  ...
  SUBROUTINE TRMTOL_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,KF_LEG)
    ...
    USE ISO_C_BINDING,          ONLY: C_SIZEOF
    USE TIMING_MOD,    ONLY: TPACK3, TPACK4, &
...
!! #ifdef OMPGPU
    ...
    !$OMP& PRIVATE(KMLOC,JGL,JK,KM,ISL,IGLS,OFFSET1,OFFSET2,ZAOA,ZSOA)
...
USE ISO_C_BINDING,          ONLY: C_SIZEOF
!
USE TIMING_MOD,    ONLY: TUNPK3, TUNPK4, &
&                      TCOUNT, T_EVENT, T_BATCH, T_STAGE, T_TYPE, GET_TIME, TENABLE
...
!$OMP& PRIVATE(KGL,JF,JM,IGLG,RET_REAL,RET_COMPLEX,ISTA)
!$OMP DO SCHEDULE(STATIC)
#elif defined (OMPGPU)
!! #ifdef OMPGPU
! Directive incomplete -> putting more variables in SHARED() triggers internal compiler error
...
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) &
!$OMP& SHARED(D,G,KF_CURRENT,ILOEN_MAX,OFFSET_VAR,FOUBUF,PREEL_COMPLEX) &
!$OMP& PRIVATE(IGLG,RET_REAL,RET_COMPLEX,ISTA) &
