...
#if defined (HOSTGPU)
    !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(IGLG,IOFF_LAT,ISTA,SCAL) &
    !$OMP& SHARED(D,R,KF_FS,OFFSET_VAR,G,&
    !$OMP& PREEL_COMPLEX,FOUBUF_IN)
#elif defined (OMPGPU)
!! #ifdef OMPGPU
...
#if defined (HOSTGPU)
    !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(KM) &
    !$OMP& SHARED(D_NUMP,KF_FS,D_MYMS,G_NDGLU,D_OFFSETS_GEMM1,IIN_STRIDES0,ZINPA,ZINPS)
#elif defined (OMPGPU)
!! #ifdef OMPGPU
