...
!! #ifdef OMPGPU
      ...
      ELSE
#if defined (HOSTGPU)
        !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,&
  ...
  TCOUNT = TCOUNT + 1
    ...
    IF(LLOCAL_CONTRIBUTION)THEN
      ...
      IF(PRESENT(PGP)) THEN
        ...
        !$OMP& IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,PGP,PREEL_REAL)
#elif defined (OMPGPU)
!! #ifdef OMPGPU
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,&
        !$OMP& IPOS) SHARED(KF_FS,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,IFLDA,&
...
#if defined (HOSTGPU)
      !$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(IPOS) &
